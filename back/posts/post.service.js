...
const createPost = async (req, res) => {
  ...
  if (!title || !content)
    return res.status(400).json({ message: 'wrong parameteres' });
...
const deletePostById = async (req, res) => {
  ...
  if (!deletedPost)
    return res.status(400).json({ message: `Cannot delete post by id: ${id}` });
...
const updatePostById = async (req, res) => {
  ...
  const updatedPost = await postModel.findByIdAndUpdate(id, updateRequest, {
    new: true,
  });
